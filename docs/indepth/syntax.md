...
右論理シフトを8回行うことで、数値の下位バイトを破棄できます。
...
STA $01
```
アドレスや数値の特定部分のみを取得したい場合、論理シフトは非常に有用となります。
...
### アドレスの構成
論理シフトとオペコード長宣言子を使用することで、特定のサブルーチンを指定するアドレスやインダイレクトアドレスを構成することが可能になります。
